...
class BillingPage(QWidget):
    ...
    def load_clients(self):
        ...
        cur.execute("""
            SELECT * FROM clients
            WHERE LOWER(type) IN ('apartment', 'household')
            AND status = 'Active'
            ORDER BY name
        """)
        rows = cur.fetchall()
    ...
    def compute_charge(self, billing_type, usage):
        ...
        if billing_type == "Commercial":
            base = float(self.get_setting(cur, "COM_BASE", 300))
            threshold = float(self.get_setting(cur, "COM_THRESHOLD", 10))
            rate = float(self.get_setting(cur, "COM_RATE", 50))
        else:
            base = float(self.get_setting(cur, "RES_BASE", 150))
            threshold = float(self.get_setting(cur, "RES_THRESHOLD", 10))
            rate = float(self.get_setting(cur, "RES_RATE", 35))

        conn.close()

        if usage <= threshold:
            return base
        return base + (usage - threshold) * rate
